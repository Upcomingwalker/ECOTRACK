...
// we'll treat carbonSaved as positive indicator (higher = better)
function animateRing(){
  ...
  // also tweak stroke color a little based on pct
...
// render missions: we'll define a set with reward logic
const MISSION_DEFS = [
...
function renderMissions(){
  ...
  MISSION_DEFS.forEach(m=>{
    ...
    btn.addEventListener('click', ()=>{
      ...
      // unlock persistent badges or daily timer handled elsewhere
    });
...
function populateLeaderboard(){
  ...
  // already has placeholder items in HTML; this updates "You"
  document.getElementById('lbYou').textContent = state.xp + (state.carbonSaved * 2);
}
...
// navigation (simple tab behaviour)
document.querySelectorAll('.nav-btn').forEach(btn=>{
  btn.addEventListener('click', ()=>{
    document.querySelectorAll('.nav-btn').forEach(b=>b.classList.remove('active'));
    btn.classList.add('active');
    // simple UI switch - not hiding sections fully to keep things snappy
    const tab = btn.dataset.tab;
    if(tab === 'missions') { document.getElementById('contentArea').scrollIntoView({behavior:'smooth'}) }
    else if(tab === 'badges') { document.querySelector('.badges-panel').scrollIntoView({behavior:'smooth'}) }
    else if(tab === 'stats') { alert('Stats panel coming soon — import your real emissions data for precise tracking.'); }
  });
});
...
// fun: animate small confetti when we cross big milestones
...
function burstConfetti(){
  ...
  for(let i=0;i<count;i++){
    ...
    Object.assign(d.style,{
      position:'fixed',left:`${Math.random()*80 + 10}%`,top:`${Math.random()*40 + 10}%`,
      width:`${size}px`,height:`${size}px`,background:`hsl(${Math.random()*80+90}deg 60% 60%)`,
      transform:`rotate(${Math.random()*360}deg)`,opacity:1,borderRadius:'2px',zIndex:9999
    });
    document.body.appendChild(d);
    setTimeout(()=>{ d.style.transition='transform 1.2s ease, top 1.2s ease, opacity 1.2s'; d.style.top = `${window.innerHeight + 50}px`; d.style.opacity = '0'; d.style.transform = `translateX(${(Math.random()-0.5)*200}px) rotate(${Math.random()*720}deg)` },30);
    setTimeout(()=>d.remove(),1500)
  }
}

// small CSS for confetti (inserted in JS for single-file portability)
